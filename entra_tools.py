...
def check_group_ownership_escalations():
    ...
    headers = _get_headers()
    # Fix: Expand only 'owners' in the primary query to satisfy Graph API constraints
    url = "https://graph.microsoft.com/v1.0/groups?$select=id,displayName,isAssignableToRole&$expand=owners"
    res = requests.get(url, headers=headers)
    if res.status_code != 200: return {"error": res.text}
    ...
    for g in groups:
        ...
        if owners:
            ...
            # Query members in a distinct API call
            group_id = g.get("id")
            members_res = requests.get(f"https://graph.microsoft.com/v1.0/groups/{group_id}/members?$select=id", headers=headers)
            member_count = len(members_res.json().get("value", [])) if members_res.status_code == 200 else 0
